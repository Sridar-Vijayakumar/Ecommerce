...
const ProductCard = ({ product, badge }) => {
  ...
  const savedPercent = oldPrice > price
    ? Math.round(((oldPrice - price) / oldPrice) * 100)
    : 0;
  const discount = Math.min(100, Math.max(0, Number(product.discount) || savedPercent));
  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white transition duration-300 hover:-translate-y-1 hover:shadow-soft">
      {cardBadge && <span className="absolute left-4 top-4 z-10 rounded-full bg-coral-500 px-3 py-1 text-[11px] font-extrabold uppercase tracking-wider text-white">{cardBadge}</span>}
        ...
        <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
          <div>
            <p className="text-xl font-black text-ink-900">
              ₹{Number(salePrice.toFixed(2)).toLocaleString("en-IN")}
            </p>
